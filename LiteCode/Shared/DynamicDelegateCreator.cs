...
namespace LiteCode.Shared
{
    internal class DynamicDelegateCreator
    {
        ...
        /*public static Type CreateDelegateType(SharedDelegate sharedDel)
        {
            TypeBuilder typeBuilder = modBuilder.DefineType("_Del_Type_" + IncrementName(),
            ...
            //create constructor
            ConstructorBuilder constructor = typeBuilder.DefineConstructor(MethodAttributes.Public | MethodAttributes.HideBySig | 
                                                                             MethodAttributes.SpecialName | MethodAttributes.RTSpecialName,
                                                                             CallingConventions.HasThis, new Type[] { typeof(object), typeof(IntPtr) });
            constructor.GetILGenerator().Emit(OpCodes.Ret);

            //Create virtual method - BeginInvoke
            List<Type> types = new List<Type>();
            types.AddRange(sharedDel.sharedMethod.ArgumentTypes);
            types.Add(typeof(AsyncCallback)); //callback
            types.Add(typeof(object));
            typeBuilder.DefineMethod("BeginInvoke", MethodAttributes.Public | MethodAttributes.HideBySig | MethodAttributes.NewSlot | MethodAttributes.Virtual,
                                     CallingConventions.HasThis, typeof(IAsyncResult), types.ToArray()).GetILGenerator().Emit(OpCodes.Ret);

            //Create virtual method - EndInvoke
            typeBuilder.DefineMethod("EndInvoke", MethodAttributes.Public | MethodAttributes.HideBySig | MethodAttributes.NewSlot | MethodAttributes.Virtual,
                                     CallingConventions.HasThis, typeof(void), new Type[] { typeof(IAsyncResult) }).GetILGenerator().Emit(OpCodes.Ret);

            //Create virtual method - Invoke
            typeBuilder.DefineMethod("Invoke", MethodAttributes.Public | MethodAttributes.HideBySig | MethodAttributes.NewSlot | MethodAttributes.Virtual,
                                     CallingConventions.HasThis, typeof(void), sharedDel.sharedMethod.ArgumentTypes).GetILGenerator().Emit(OpCodes.Ret);
